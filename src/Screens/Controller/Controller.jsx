  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      return setControladores({
        ...
        services: createTree(data.services),
      });
    };
    fetchData();
    setSubList([]);
  ...
  return (
    <AnimatedContainer info={item}>
      <CustomScrollViewContainer>
        <SubContainer>
          <>
            {controladores.scenes && controladores.scenes.length > 0 && (
              <ListContainer>
                <List
                  title="CENAS"
                  type="scenes"
                  items={controladores.scenes}
                  setItems={setControladores}
                  level={0}
                  activeSub={activeSub}
                  setActiveSub={setActiveSub}
            ...
            {subListType === 'controls' &&
              activeSub.length > 0 &&
              activeSubComponent}
            {controladores.sensors && controladores.sensors.length > 0 && (
              <ListContainer>
                <List
                  title="SENSORES"
                  type="sensors"
                  items={controladores.sensors}
                  setItems={setControladores}
                  level={0}
                  activeSub={activeSub}
                  setActiveSub={setActiveSub}
            ...
            {controladores.services && controladores.services.length > 0 && (
              <ListContainer>
                <List
                  title="SERVIÇOS"
                  type="services"
                  items={controladores.services}
                  setItems={setControladores}
                  level={0}
                  activeSub={activeSub}
                  setActiveSub={setActiveSub}
